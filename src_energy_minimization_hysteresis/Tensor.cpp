...
void Tensor::init_all_up() // Initialization with all spins up
{
    spins.clear(); // We remove all previous elements
...
double Tensor::getDeltaE(int x, int y, int z, double J0, double J1, double J2) // Computes and returns the energy variation of the system when a spin is switch (from up to down or down to up)
{
    ...
    There is a *2 because Einitial = -Efinal, thus DeltaE = 2 * Efinal = - 2 * Einitial
...
double Tensor::getDeltaE_zeeman(int x, int y, int z, double J0, double J1, double J2, double H) // Idem with the zeeman term -g*mu_0*mu_B*H*S
{
    /* We add - 2 * H * Spin_final or 2 * H * Spin_initial */
